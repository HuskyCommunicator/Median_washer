...
class RuleTab(ctk.CTkFrame):
    ...
    def _init_ui(self):
        ...
        # 第1行：主要编辑
        self.btn_advanced = ctk.CTkButton(self.frame_rule_ops, text="📝 编辑详情(JSON)", height=40, fg_color="#555555", command=self.app.open_advanced_editor)
        self.btn_advanced.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
        
        # 第2行：新建与重命名
        self.btn_new_rule = ctk.CTkButton(self.frame_rule_ops, text="➕ 新增规则", height=35, command=self.app.create_new_rule)
        ...
        self.btn_load_def = ctk.CTkButton(self.frame_rule_ops, text="📥 导入默认库", height=35, fg_color="#333333", command=self.app.load_defaults)
